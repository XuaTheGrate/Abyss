...
from cogs.utils import battle as bt, formats
...
```""")
            self.bot.log.warning(f"task died with {exc}")
            self._task = self.bot.loop.create_task(self.task_kill())

    async def cog_command_error(self, ctx, error, battle=None):  # pylint: disable=arguments-differ
        if battle:
            await self._queue.put(battle.players[0].owner.id)
            if not error:
                return
            msg = f""">>> Error occured during battle.
{battle.players}
...
```"""
            ...
            return self.bot.send_error(msg)
    ...
    async def _encounter(self, ctx, *names):
        ...
        for name in names:
            ...
            enemy = bt.Enemy(**encounter)
            await enemy.populate_skills(self.bot)
            enemy.skills.remove(self.bot.players.skill_cache['Guard'])
    ...
    async def encounter(self, ctx, *, ___=None, force=False):
        ...
        enemies = [await bt.Enemy(**e, bot=self.bot).populate_skills(self.bot) for e in enc]
    ...
    async def pvp(self, ctx, *, user: discord.Member):
        try:
            player2 = self.bot.players.players[user.id]
        except KeyError:
            # todo: cache the player
            return await ctx.send("users player is not loaded.")
        battle = bt.PVPBattle(ctx, teama=(ctx.player,), teamb=(player2,))
    ...
    async def new(self, ctx):
        ...
        task = self.bot.loop.create_task(ctx.send_as_paginator(data, codeblock=True))
        nvalid = list(map(str.lower, valid))
        choice = None

        while True:
            try:
                select = await self.bot.wait_for("message", check=lambda m: m.author == ctx.author and
                                                 m.channel == ctx.channel, timeout=60)
        ...
        enemy_data = {'name': demondata['name'], 'desc': demondata['desc']}
        embed = discord.Embed(title=demondata['name'], description=demondata['desc'])
        await ctx.send(f"Great, let's use {demondata['name']}. "
                       "Next you need to assign a level. Any number between 1-99 is valid.")
        msg = await self.bot.wait_for("message", check=lambda m: m.content.isdigit() and 0 < int(m.content) < 100)
        embed.add_field(name='Level', value=msg.content)
        enemy_data['level'] = int(msg.content)
